...
func (m *Mongo) Rooms() drivers.RoomsRepository {
	if m.roomsRepository != nil {
		return m.roomsRepository
	}

	m.roomsRepository = &RoomsRepository{
		collection: m.DB.Collection(CollectionRooms),
	}
	return m.roomsRepository
}

func (m *Mongo) Places() drivers.PlacesRepository {
	if m.placesRepository != nil {
		return m.placesRepository
	}

	m.placesRepository = &PlacesRepository{
		collection: m.DB.Collection(CollectionRooms),
	}
	return m.placesRepository
}
...
func (m *Mongo) Close(ctx context.Context) error {
	log.Printf("Disconnected from MongoDB: %s", m.dbName)
