...
GOFLAGS=-a -tags conductor -installsuffix conductor -mod=vendor

run:
	go run $(BUILD_PATH)/main.go

bin:
	echo "  >  Building binary \"conductor\" $(VERSION)..."
	go build $(GOFLAGS) $(LDFLAGS) -o $(BINARY) $(BUILD_PATH)

bin-linux:
	echo "  >  Building linux-amd64 binary \"conductor\" $(VERSION)..."
	GOOS=linux GOARCH=amd64 go build $(GOFLAGS) $(LDFLAGS) -o $(BINARY)-linux $(BUILD_PATH)

bin-windows:
	echo "  >  Building windows-amd64 binary \"conductor\" $(VERSION)..."
...
.PHONY: all bin bin-linux bin-windows build clean coverage rebuild run unit
